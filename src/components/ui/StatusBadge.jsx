const statusConfig = {
  Abierto: { color: 'bg-blue-100 text-blue-700 border-blue-200' },
  'En Proceso': { color: 'bg-amber-100 text-amber-700 border-amber-200' },
...
export default function StatusBadge({ status }) {
  ...
  return <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium border ${config.color}`}>{status}</span>;
}
